...
const TrendingNewsThumbnail: React.FunctionComponent<Props> = ({
  post,
  big,
}) => {
  ...
  const getCategory = async () => {
    try {
      ...
      const res = await fetch(
        `${process.env.BASE_URL}/api/posts/category/getById?id=${post.categoryId}`,
        {
          method: "GET",
          cache: "no-cache",
        }
        ...
        {big ? (
          <div className="relative w-full h-full">
            ...
            <div className="absolute h-full w-full opacity-100 flex flex-col">
              <div
                ...
                style={{
                  backgroundImage:
                    "linear-gradient(180deg, rgba(0,0,0,0.2), black)",
                }}
              ></div>
            ...
            <div className="absolute h-full container bg-transparent">
              <div className="h-full w-full md:pl-5 md:pr-5 flex flex-col justify-end">
                <h1
                  className={`mb-0 ml-0 line-clamp-3
                            text-3xl
                            font-bold
                            md:text-2xl
                            lg:text-3xl
                ...
                {big && (
                  <p className="text-xs w-[70%] lg:text-sm mb-3 mt-0.5 text-gray-400  line-clamp-2">
                    {post.subtitle}
                  </p>
                )}
                ...
                <div className="flex w-full items-center mb-5 mt-2">
                            ...
                            category.name === "Novosti"
                              ...
                              : category.name === "Sport"
                                ...
                                : category.name === "Kultura"
                                  ? "bg-purple-700"
                                  : category.name === "Društvo"
                                    ? "bg-[#4e85c7]"
                                    : category.name === "Politika"
                                      ? "bg-red-500"
                                      : "bg-white"
                          }`}
                        ></div>
                  ...
                  <p className="md:text-xs text-gray-400 text-xs">
          ...
          <div className="h-full mt-0 mb-0 w-full bg-transparent rounded-0 flex flex-col items-center justify-center border-b-[1px] border-solid border-secondary">
            <div className="container h-full flex flex-col justify-center">
              <h1
                className={`mb-1 ml-0 line-clamp-2
                            text-sm
                            md:text-2xl
                            lg:text-3xl
                    `}
              ...
              <div className="flex w-full items-center mb-0">
                ...
                <p className="md:text-xs hidden text-gray-400 text-xs">
                  <TrendingNewsDate date={post.createdAt} />
                </p>
              </div>
            </div>
      ...
      <div className="h-full w-ful hidden md:block">
        <div className="relative w-full h-full">
          ...
          <div className="absolute h-full w-full bg-transparent">
            <div
              className={`h-full w-full pl-3 md:pl-5 md:pr-5 flex flex-col justify-end ${
                big ? "md:pr-20" : "md:pr-5"
              }`}
            >
              <h1
                className={`ml-0
              ...
              {big && (
                <p className="text-xs lg:text-xs mb-2 text-gray-400 dark:text-gray-400 line-clamp-2">
                  {post.subtitle}
                </p>
              )}
              <div
                className={`flex items-center mt-1 ${big ? "mb-5" : "mb-3"} `}
              >
                <div className={`flex flex-[1] items-center mt-0.5`}>
                  {category ? (
                    <div className="flex mb-0 items-center mt-0">
                      ...
                      ></div>
                      <p
                        className={`text-sm max-[700px]:text-xs ${
                          big ? `text-white` : `text-gray-400`
                        }  `}
                      >
                        {category.name}
                      </p>
                ...
                <p
                  className={`md:text-xs text-gray-400 text-xs 
                            ${big ? "" : ""}
                        `}
                >
                  <TrendingNewsDate date={post.createdAt} />
