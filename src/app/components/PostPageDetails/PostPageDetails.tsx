...
const getAllSubcategorys = async (categoryId: string) => {
  const res = await fetch(
    `${process.env.BASE_URL}/api/posts/category/getSubcategorys?id=${categoryId}`,
    {
      method: "GET",
      cache: "no-cache",
    }
...
const PostPageDetails: React.FunctionComponent<Props> = async ({
  ...
  const subcategorysData: Promise<Subcategory[]> =
    ...
    <div className={`h-28 mb-10`}>
      <div className="dark:bg-secondary bg-white flex flex-col h-full">
        <Link
          ...
          className="flex-[4] flex items-center justify-start border-[#333] border-b-[0px] border-solid"
        >
          ...
          <ul className="flex w-full h-full justify-start items-center overflow-x-scroll lg:overflow-x-hidden no-scrollbar">
            {first[0] && (
              <Link
                ...
                style={{
                  background:
                    first[0]?.name === subcategory?.name
                      ? `${category.hexCol}`
                      : "",
                }}
                className={`h-full transition mr-0 hover:bg-[${categoryHex}] hover:dark:bg-[${categoryHex}]
                                    min-w-fit
                                    px-1
                                    ${
                                      first[0]?.name === subcategory?.name
            ...
            {other.map((subcategoryCard) => (
              <Link
                ...
                style={{
                  background:
                    subcategoryCard.name === subcategory?.name
                      ? `${category.hexCol}`
                      : "",
                }}
                className={`h-full transition mr-0 hover:bg-[${categoryHex}] hover:dark:bg-[${categoryHex}]
...
export const PostPageDetailsLoading = () => {
  return <Skeleton className="h-20 w-full"></Skeleton>;
};
