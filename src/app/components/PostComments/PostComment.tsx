...
const PostComment = ({ comment, postId }: Props) => {
  ...
  const onDelete = async (commentId: string) => {
    try {
      const res = await fetch(`${process.env.BASE_URL}/api/admin/comments`, {
        method: "DELETE",
  ...
  return (
    <div
      key={comment.id}
      className="border-t-[1px] border-solid dark:border-secondary pt-5 pb-3 bg-secondary dark:bg-black"
    >
            ...
            <div className="mt-1 mb-1 ml-3 flex items-center">
              ...
              <div className="">
                {comment.replies.length > 0 && (
                  <div
                    className="mt-0 ml-0 flex pr-3 py-2 max-w-fit cursor-pointer rounded-3xl"
                    onClick={() => {
                  ...
                  >
                    ...
                    <p className="text-xs text-primary flex">
                      Odgovora:
                      <span className="ml-1">
                        {isSubmitting ? (
                          <Loader2 className="h-[20px] w-[20px]" />
                        ) : (
                ...
                {comment.replies?.map((reply) => (
                  <div key={reply.id} className="mb-5 flex">
                    ...
                    <div className="flex-[1]">
                      ...
                      <div className="mt-1 ml-3">
                        <p
                          className="text-sm whitespace-normal max-w-[260px] md:max-w-lg lg:max-w-4xl"
