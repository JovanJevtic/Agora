...
import {
  TSCommentCreationForm,
  commentCreationFormSchema,
...
type Props = {
  ...
  postSlug: string;
  // createComment: (postId: string, isReply: boolean, parrentCommentId: string | null, formData: FormData) => Promise<any | Comment>;
  // addOptimistic: (comm: string) => void;
};

const AddCommentForm = ({ postId, comments, postSlug }: Props) => {
  ...
  const submitAction = async (formData: FormData) => {
    ...
    const createFullComment = createComment.bind(
      null,
      postId,
      false,
      null,
          ...
          <Dialog open={dialogOpen} onOpenChange={(arg) => setDialogOpen(arg)}>
            <DialogContent className="sm:max-w-md border-secondary">
              <DialogHeader>
                ...
                <DialogDescription>
                  Potrebno je da budete prijavljeni na Vaš račun da biste imali
                  pristup komentarisanju.
                  <p className="mt-3 text-primary">
                    <Link href={`/login`} className="underline">
                      Prijavite se ovdje uz Google na svega jedan klik!
                    </Link>
                  </p>
                </DialogDescription>
              </DialogHeader>
