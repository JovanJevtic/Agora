...
export const POST = async (request: NextRequest) => {
  try {
    ...
    const validateResponse = registerFormSchema.safeParse({
      email,
      password,
      name,
      confirmPassword,
    ...
    const emailEquieped = await prisma.user.findUnique({
      where: { email: email },
    });
    if (emailEquieped)
      return NextResponse.json(
        { errors: { email: "Email je vec u upotrebi!" } },
        { status: 400 }
      );
    ...
    if (!emailVerifyToken)
      return NextResponse.json(
        { error: "Doslo je do greske...Molimo Vas pokusajte ponovo" },
        { status: 500 }
      );
    ...
    return NextResponse.json(
      { error: "Došlo je do iznenadne greške...Molimo Vas pokušajte ponovo!" },
      { status: 500 }
    );
  }
};
